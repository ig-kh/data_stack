...
def YM2M_parse_transform(df, col): # CREDIT_HISTORY_AGE attribute
    df = df.withColumn(f"{col}_Y", F.regexp_extract_all("col", F.lit(r"(\d+) Years\s*and\s*(\d+) Months"), 1))
    df = df.withColumn(f"{col}_M", F.regexp_extract_all("col", F.lit(r"(\d+) Years\s*and\s*(\d+) Months"), 2))
    df = df.withColumn(f"{col}_transformed", F.when(F.col(f"{col}_Y") != "").otherwise(0)*12 + F.when(F.col(f"{col}_M") != "").otherwise(0))
    return df.drop(col, f"{col}_Y", f"{col}_M")

def reconstruct_by_group(df, damaged_col, reference_col):
